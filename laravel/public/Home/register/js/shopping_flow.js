...
function selectShipping(obj)
{
  ...
  //获取当前的supportCod
  var supportCod = new Boolean(jq(obj).attr('supportCod'));
  
  //如果支持COD，启用COD支付；如果不支持COD，禁用COD支付，取消COD勾选
  if(supportCod){
	//TODO
  }else{
...
function orderSelectedResponse(result)
{
  ...
  try
  {
	...
	//因为邮费刷新页面
	var before_has_shipping_fee = layer.innerHTML.indexOf('配送费用')==-1;
    layer.innerHTML = (typeof result == "object") ? result.content : result;//这句本来就有
	var after_has_shipping_fee = layer.innerHTML.indexOf('配送费用')==-1;
	if(before_has_shipping_fee != after_has_shipping_fee){
